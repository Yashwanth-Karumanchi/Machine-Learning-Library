...
class LogisticRegressionSGD:
    def __init__(self, input_size, variance):
    ...
    def compute_gradient(self, X, y):
        ...
        gradient = -np.dot(X.T, (y - predictions)) + (self.weights / self.variance)
        return gradient

    def compute_loss(self, X, y):
        predictions = self.sigmoid(np.dot(X, self.weights))
        likelihood = y * np.log(predictions + 1e-9) + (1 - y) * np.log(1 - predictions + 1e-9)
        prior = -np.sum(self.weights**2) / (2 * self.variance)
...
for variance in variances:
    model = LogisticRegressionSGD(X_train.shape[1], variance)
