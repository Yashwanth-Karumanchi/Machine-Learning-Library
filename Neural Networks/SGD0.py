...
class NeuralNetwork:
    def __init__(self, input_size, hidden_size, output_size, initialize_zero=False):
        np.random.seed(42)
        if initialize_zero:
            self.weights = {
...
for hidden_size in widths:
    print(f"\nTraining with hidden size: {hidden_size} and random initialization")
    nn_random = NeuralNetwork(X_train.shape[1], hidden_size, 1)
    learning_curve_random = nn_random.train(X_train, y_train, hidden_size, gamma_0, d, epochs)

    train_error_random = 1 - np.mean(nn_random.predict(X_train) == y_train)
    test_error_random = 1 - np.mean(nn_random.predict(X_test) == y_test)

    print(f"Random Initialization - Training Error: {train_error_random:.4f} | Test Error: {test_error_random:.4f}")
